document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('paymentSummary').innerHTML = `<h3 style="margin-top:0;color:var(--brown)">ยอดที่ต้องชำระ</h3>${draft.items.map(i=>`<div class="summary-line"><span>${BookApp.escapeHtml(i.title)} × ${i.qty}</span><strong>${BookApp.formatTHB(i.price*i.qty)}</strong></div>`).join('')}<div class="summary-line"><span>ค่าจัดส่ง</span><strong>${BookApp.formatTHB(draft.shipping)}</strong></div><div class="summary-line total"><span>ยอดสุทธิ</span><span>${BookApp.formatTHB(draft.total)}</span></div><div class="status-panel"><strong>หลังส่งสลิป</strong><br><span class="helper">สถานะจะเป็น “รอตรวจสลิป” และสินค้าเป็น “จองสินค้า”</span></div>`;
