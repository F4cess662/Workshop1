...
function render(){
  ...
  const selectedShipping = shippingOptions.some(s=>s.id===localStorage.getItem(BookApp.STORAGE.selectedShipping)) ? localStorage.getItem(BookApp.STORAGE.selectedShipping) : 'standard';
  localStorage.setItem(BookApp.STORAGE.selectedShipping, selectedShipping);
  const ship = shippingOptions.find(s=>s.id===selectedShipping);
