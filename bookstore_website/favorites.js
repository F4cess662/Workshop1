document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateSelectionControls(items) {
    const selectableIds = items.filter(p => BookApp.availableStock(p) > 0).map(p => p.id);
  ...
  function render() {
    ...
    grid.querySelectorAll('.book-card').forEach(card => {
      ...
      card.insertAdjacentHTML('afterbegin', `<label class="favorite-select"><input type="checkbox" data-favorite-select="${product.id}" ${selected.has(product.id) ? 'checked' : ''}><span class="sr-only">เลือก ${BookApp.escapeHtml(product.title)}</span></label>`);
      card.classList.toggle('is-selected', selected.has(product.id));
    });
  ...
  selectAll.addEventListener('change', () => {
    const items = BookApp.products().filter(p => BookApp.favorites().includes(p.id) && BookApp.availableStock(p) > 0);
    items.forEach(product => selectAll.checked ? selected.add(product.id) : selected.delete(product.id));
    render();
  });
